...
- **Integración M2M real**: El endpoint `POST /api/shipping/dispatch` notifica automaticamente a la Seller App enviando el `trackingId` a `{SELLER_SERVICE_URL}/api/orders/{orderRef}/tracking`. Al confirmar entrega, la Server Action `confirmDelivery` envia una notificación a `{PAYMENTS_API_URL}/api/payments/{transaction_id}/delivered` con el header `x-service-key`. Ambas llamadas usan `try/catch` independiente de la transacción local de Prisma para garantizar que el flujo principal nunca se bloquee por fallos externos (eventual consistency).
